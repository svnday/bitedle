...
export async function GET(request: NextRequest) {
  ...
  if (guildId) {
    ...
    // Opening the Activity is a launch — record it so this guild's live
    // preview scopes to this window's players. Awaited before the after()
    // render below.
    await store.recordLaunch(date, identity.id, guildId, Date.now());
